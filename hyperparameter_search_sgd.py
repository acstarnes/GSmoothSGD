...
parser.add_argument('-s', '--seed', default=2024, type=int,
                    help='Seed used for CV split.')
args = parser.parse_args()

dataset = args.dataset
optimizer = args.optimizer

# Load training data (Note: for the hyperparameter search, we use k-fold cross validation and do not use test dataset.)
x_train_full,y_train_full,_,_,y_train_sparse_full,_ = load_environment(dataset)

rng = np.random.default_rng(seed=args.seed)
train_index = list(range(len(x_train_full)))
rng.shuffle(train_index)
folds = np.array_split(train_index, 5)


if not os.path.isfile(f'hyperparameter_search_{dataset}_{optimizer}_batsz1.csv'):
    print('Creating log file')
    pd.DataFrame(
        data = None,
        columns = ['seed', 'k', 'accuracy', 'loss', 'val_accuracy', 'val_loss', 's', 'lr','l1', 'l3', 'l4', 'l4_h']
    ).to_csv(f'hyperparameter_search_{dataset}_{optimizer}_batsz1.csv', index = False)

if not os.path.isfile(f'initial_cnn.weights.h5'):
    # Create initial smooth CNN
    build_smooth_cnn(sigma = sigma, input_shape=x_train_full[0].shape, regularizer_strength=[1e-5,1e-5,1e-5,1e-5], learning_rate = 0.01).save_weights('initial_cnn.weights.h5')

tf.keras.backend.clear_session()
for reg in tqdm(list(product([1.,0.5,0.1],[0.001,0.0001],[1e-5,1e-7],[1e-5,1e-7],[1e-5,1e-7],[1e-5,1e-7]))[0:]):
    print(reg)

    sigma = reg[0]

    for fold in tqdm(range(5), desc = 'CV'):
        print('\n')

        sCNN = build_smooth_cnn(sigma = sigma, input_shape=x_train_full[0].shape, regularizer_strength=reg[2:], learning_rate = reg[1])
        sCNN.load_weights('initial_cnn.weights.h5')
        ...
        sCNN.fit(
            x_train_full[np.delete(folds,fold,0).flatten()],
            y_train_sparse_full[np.delete(folds,fold,0).flatten()],
            epochs = 25,
            batch_size = 1,
            validation_data = (
                x_train_full[folds[fold]],
                y_train_sparse_full[folds[fold]]
            ),
