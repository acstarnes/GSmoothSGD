...
class ConvolutionRegularizer(tf.keras.regularizers.Regularizer):
    ...
    def __init__(self,conv_layer,strength = 0.01,scale = 1.):
        self.strength = strength
        self.scale = scale
...
def build_cnn(
    ...
    model.add(Input(input_shape, name = 'x0_input'))

    model.add(Conv2D(32,4,1,'valid', activation = 'linear'))
...
def build_smooth_cnn(
    sigma = 1.,
    regularizer_strength = [1e-7,1e-7,1e-7,1e-7,1e-7,1e-7,1e-7,1e-7,1e-7,1e-7,1e-7,1e-7],
    input_shape = (28,28,1),
    learning_rate = 1e-4,
    opt = 'adam'
):
    ...
    # Create Convolutional Regularizer Weights
    initializer = Constant(1.)

    l02_reg = Conv2D(32,4,1,'valid', kernel_initializer=initializer)
    l04_reg = Conv2D(64,4,1,'valid', kernel_initializer=initializer)
    l05_reg = Conv2D(64,4,1,'valid', kernel_initializer=initializer)

    _ = l02_reg(Input((29,29,32)))
    _ = l04_reg(Input((13,13,32)))
    _ = l05_reg(Input((10,10,64)))

    model = Sequential()
    
    model.add(Input(input_shape, name = 'x0_input'))

    model.add(Conv2D(32,4,1,'valid', activation = 'linear', activity_regularizer=ReluRegularizer(regularizer_strength[0])))
    model.add(Activation('relu', activity_regularizer=ConvolutionRegularizer(l02_reg,regularizer_strength[1], 0.5 * sigma ** 2)))
    model.add(Conv2D(32,4,1,'valid', activation = 'linear', activity_regularizer=ReluRegularizer(regularizer_strength[2]), kernel_regularizer=NormRegularizer(regularizer_strength[1], 62.5 * sigma ** 2)))
    model.add(Activation('relu'))
    model.add(AveragePooling2D(2))

    model.add(Flatten(activity_regularizer=NormRegularizer(regularizer_strength[3], 128 * sigma ** 2)))
    model.add(Dense(256, activation = 'linear', activity_regularizer=ReluRegularizer(regularizer_strength[4]), kernel_regularizer=NormRegularizer(regularizer_strength[3], 0.5 * sigma ** 2)))
    model.add(Activation('relu', activity_regularizer=NormRegularizer(regularizer_strength[5], 3 * sigma ** 2)))
    model.add(Dense(10, activation = 'linear', kernel_regularizer=NormRegularizer(regularizer_strength[5], 0.5 * sigma ** 2)))
...
cifar_regularizers = [1e-5,1e-5,1e-5,1e-5,1e-5,1e-5]
...
scnn_init = build_smooth_cnn(
    ...
    regularizer_strength=cifar_regularizers,
    learning_rate=0.1
)
...
for seed in tqdm(rng.integers(low=0,high=1e8,size=1), desc = 'Seed'):
    ...
    for noise in tqdm(product([0,0.25,0.5,0.75,1.],[0,0.1,0.2,0.3,0.4]), desc = 'Noise', leave = False):
        print('\n')
        ...
        for s in tqdm([1.,0.5,0.1,0.01], desc = 'Sigma', leave = False):
            ...
            if s == 1.:
                regularizers = [1e-5,1e-7,1e-7,1e-5,1e-7,1e-5]
                learning_rate = 1e-4
            elif s == 0.5:
                regularizers = [1e-7,1e-7,1e-7,1e-5,1e-7,1e-7]
                learning_rate = 1e-4
            elif s == 0.1:
                regularizers = [1e-5,1e-7,1e-7,1e-5,1e-5,1e-7]
                learning_rate = 1e-4
            elif s == 0.01:
                regularizers = [1e-5,1e-7,1e-7,1e-5,1e-5,1e-7]
                learning_rate = 1e-4
            else:
                regularizers = [1e-5,1e-5,1e-5,1e-5,1e-5,1e-5]
                learning_rate = 1e-5
